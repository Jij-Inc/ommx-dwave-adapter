...
def build_one_hot_instance(
    ...
    Minimize:
        sum_{group, choice} cost[group, choice] * x[group, choice]
    Subject to:
        sum_choice x[group, choice] = 1 for each group
    ...
    if special_constraints != "none" and formulation != "one-hot":
        raise ValueError("Special-constraint workloads require the one-hot formulation")
    ...
    objective = Linear(
        terms={
            variable_id(group, choice): random_generator.uniform(0.5, 1.5)
            for group in range(size)
            for choice in range(size)
        }
    )
    ...
    if special_constraints != "none":
        # Match the OMMX v3 beta3 lowering formulas used by the direct and
        # prepared comparison workloads.
